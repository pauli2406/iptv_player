...
class EpgItem {
  ...
  factory EpgItem.fromProgramme(
      Programme programme, String channelId, IptvServer server) {
    return EpgItem(
      title: programme.titles.isNotEmpty ? programme.titles.first.value : null,
      description:
          programme.descs.isNotEmpty ? programme.descs.first.value : null,
      start: programme.start,
      end: programme.stop ??
          programme.start.add(
              const Duration(hours: 1)), // Default to 1 hour if no end time
      channelId: channelId,
